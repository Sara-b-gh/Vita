...
public class AdminEvenementController {
    ...
    @FXML private ListView<Evenn> evenementListView;
    @FXML private ListView<Evenn> pastEventsListView;
    ...
    private void chargerDonnees() {
        try {
            ...
            List<Evenn> filtered = currentView == ViewMode.STATS
                    ? all
    ...
    private List<Evenn> filtrerEvenements(List<Evenn> source, LocalDateTime now) {
    ...
    private boolean matchesStatusFilter(Evenn ev, String filter, LocalDateTime now) {
    ...
    private void remplirGrille(List<Evenn> events) {
    ...
    private void remplirListe(List<Evenn> events) {
        if (evenementListView != null) {
            evenementListView.setItems(FXCollections.observableArrayList(events));
        }
    }

    private void remplirArchive(List<Evenn> all, LocalDateTime now) {
        if (pastEventsListView == null || currentView == ViewMode.STATS) return;

        List<Evenn> passes = all.stream()
                .filter(e -> e != null)
    ...
    private void mettreAJourStatistiquesEvenements(List<Evenn> baseList, List<Evenn> filteredList) {
    ...
    private void drawMonthlyHistogram(Canvas canvas, List<Evenn> allEvents) {
    ...
    private VBox creerCarteAdminCompacte(Evenn ev) {
    ...
    private void configurerListe() {
        evenementListView.setCellFactory(lv -> new ListCell<>() {
            @Override
            protected void updateItem(Evenn ev, boolean empty) {
    ...
    private void ouvrirFormulaire(Evenn evEdit) {
        ...
        ComboBox<Ressource> cbRessource = new ComboBox<>();
    ...
    private void ouvrirModalDetails(Evenn ev) {
        if (ev == null) return;
    ...
    private void supprimerAvecConfirmation(Evenn ev) {
        if (ev == null) return;
    ...
    private void ouvrirGestionReservations(Evenn ev) {
        try {
    ...
    private void afficherCarte(Evenn ev) {
    ...
    private void genererQREvent(Evenn ev) {
        if (ev == null) return;
    ...
    private void partagerEvenement(Evenn ev) {
        if (ev == null) return;
    ...
    public void ouvrirScannerQR() {
        try {
            ...
            ScannerQRCodeController controller = loader.getController();
    ...
    private void exporterFicheTxt(Evenn ev) {
        if (ev == null) return;
    ...
    public void exporterCSV() {
        ...
        try {
            ...
            List<Evenn> visibleEvents = currentView == ViewMode.STATS
                    ? all
